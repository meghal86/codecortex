...
/**
 * Maps raw entry/terminal function names to business logic domains
 */
function generateBusinessOutcome(entryName: string, terminalName: string): string {
  const combined = `${entryName} ${terminalName}`.toLowerCase();

  if (combined.includes('auth') || combined.includes('login') || combined.includes('signup') || combined.includes('token')) {
    return 'Authentication & Identity Flow';
  }
  if (combined.includes('pay') || combined.includes('checkout') || combined.includes('stripe') || combined.includes('bill')) {
    return 'Payment & Billing Process';
  }
  if (combined.includes('user') || combined.includes('profile') || combined.includes('account')) {
    return 'User Account Management';
  }
  if (combined.includes('data') || combined.includes('sync') || combined.includes('fetch') || combined.includes('load')) {
    return 'Data Synchronization & Fetching';
  }
  if (combined.includes('email') || combined.includes('notify') || combined.includes('alert') || combined.includes('message')) {
    return 'Notification & Messaging Flow';
  }
  if (combined.includes('search') || combined.includes('query') || combined.includes('filter')) {
    return 'Search & Filter Operations';
  }

  // Default fallback if no specific business domain is matched
  return `Core Execution: ${entryName.substring(0, 20)}...`;
}
...
export const processProcesses = async (
  ...
  limitedTraces.forEach((trace, idx) => {
    ...
    // AI/Heuristic Business Logic Label
    const businessLabel = generateBusinessOutcome(entryName, terminalName);
    const heuristicLabel = `${capitalize(entryName)} → ${capitalize(terminalName)} (${businessLabel})`;

    const processId = `proc_${idx}_${sanitizeId(entryName)}`;

    processes.push({
      id: processId,
      label: businessLabel, // Use business label as primary label
      heuristicLabel,
      processType,
