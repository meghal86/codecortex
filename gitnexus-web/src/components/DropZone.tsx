...
export const DropZone = ({ onFileSelect, onGitClone, onServerConnect }: DropZoneProps) => {
  ...
  const handleGitClone = async () => {
    ...
    try {
      ...
      if (!files || files.length === 0) {
        throw new Error('Repository is empty or contains no readable text files.');
      }
      ...
      if (message.includes('401') || message.includes('403') || message.includes('Authentication')) {
        ...
        setError('Repository not found, or inaccessible. Check the URL or use a PAT.');
      } else {
        setError(message);
      }
    } finally {
    ...
    <div className="flex items-center justify-center min-h-screen p-8 bg-void">
      ...
      <div className="relative w-full max-w-2xl flex flex-col items-center">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center p-3 bg-elevated rounded-2xl mb-6 shadow-glow border border-border-subtle">
            <div className="w-10 h-10 flex items-center justify-center bg-gradient-to-br from-accent to-emerald-400 rounded-xl shadow-lg text-white text-xl font-bold">
              ◇
            </div>
          </div>
          <h1 className="text-4xl font-bold text-text-primary mb-4 tracking-tight">
            Welcome to <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-emerald-400">CodeCortex</span>
          </h1>
          <p className="text-text-secondary text-lg max-w-lg mx-auto leading-relaxed">
            Understand any codebase in seconds. Upload a repository to visualize every file, function, and class as interactive <strong className="text-accent font-semibold">Nodes</strong> — then trace how they connect through imports, calls, and inheritance via <strong className="text-accent font-semibold">Edges</strong>. Perfect for onboarding, code reviews, and debugging.
          </p>
        </div>
        ...
        <div className="w-full max-w-lg">
            ...
            <div className="p-8 bg-surface border border-border-default rounded-3xl">
              ...
              <div className="space-y-3" data-form-type="other">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && !isCloning && githubUrl.trim()) {
                      handleGitClone();
                    }
                  }}
          ...
          {activeTab === 'server' && (
            <div className="p-8 bg-surface border border-border-default rounded-3xl">
              ...
              <p className="text-sm text-text-secondary text-center mb-6">
                Load a pre-built knowledge graph from a running CodeCortex server
              </p>
