...
/**
 * Fetch Subsystem analytics
 */
export const fetchAnalyticsSubsystems = async (): Promise<any> => {
  const response = await fetchWithTimeout(`${backendUrl}/api/analytics/subsystems`);
  await assertOk(response);
  return response.json();
};

/**
 * Fetch Knowledge Deserts analytics
 */
