...
export const CodeReferencesPanel = ({ onFocusNode }: CodeReferencesPanelProps) => {
  ...
  const [panelWidth, setPanelWidth] = useState<number>(() => {
    try {
      const saved = window.localStorage.getItem('codecortex.codePanelWidth');
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem('codecortex.codePanelWidth', String(panelWidth));
    } catch {
      // ignore
    }
  }, [panelWidth]);
  ...
  if (isCollapsed) {
    return (
      <aside className="h-full w-12 bg-[#0a0f18] border-x border-[#1e293b] flex flex-col items-center py-3 gap-2 flex-shrink-0">
        <button
  ...
  return (
    <aside
      ...
      className="h-full bg-[#0a0f18] flex flex-col animate-slide-in relative overflow-hidden"
    ...
    >
      ...
      />
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-[#1e293b] bg-[#0f172a]">
