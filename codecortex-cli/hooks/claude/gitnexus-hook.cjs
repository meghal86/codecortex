#!/usr/bin/env node
/**
 * CodeCortex Claude Code Hook
 *
 * PreToolUse handler — intercepts Grep/Glob/Bash searches
 * and augments with graph context from the CodeCortex index.
 *
...
/**
 * Check if a directory (or ancestor) has a .codecortex index.
 */
function findCodeCortexIndex(startDir) {
  let dir = startDir || process.cwd();
  for (let i = 0; i < 5; i++) {
    if (fs.existsSync(path.join(dir, '.codecortex'))) {
      return true;
    }
...
function main() {
  try {
    ...
    if (!findCodeCortexIndex(cwd)) return;
    ...
    // 2. require.resolve (works when codecortex is globally installed)
    // 3. Fall back to npx (works when neither is available)
    let cliPath = path.resolve(__dirname, '..', '..', 'dist', 'cli', 'index.js');
    if (!fs.existsSync(cliPath)) {
      try {
        cliPath = require.resolve('codecortex/dist/cli/index.js');
      } catch {
    ...
    console.error('CodeCortex hook error:', err.message);
  }
}
