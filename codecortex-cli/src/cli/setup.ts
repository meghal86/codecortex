 ...
 * so the CodeCortex MCP server is available in all projects.
 */
...
function getMcpEntry() {
  if (process.platform === 'win32') {
    return {
      command: 'cmd',
      args: ['/c', 'npx', '-y', 'codecortex@latest', 'mcp'],
    };
  }
  return {
    command: 'npx',
...
/**
 * Merge codecortex entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.codecortex = getMcpEntry();
  return existing;
}
...
async function setupClaudeCode(result: SetupResult): Promise<void> {
  ...
  console.log('');
  console.log('  Claude Code detected. Run this command to add CodeCortex MCP:');
  console.log('');
  console.log('    claude mcp add codecortex -- npx -y codecortex mcp');
  console.log('');
  result.configured.push('Claude Code (MCP manual step printed)');
}

/**
 * Install CodeCortex skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install CodeCortex hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the codecortex package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/codecortex/
  const destHooksDir = path.join(claudeDir, 'hooks', 'codecortex');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'codecortex-hook.cjs');
    const dest = path.join(destHooksDir, 'codecortex-hook.cjs');
    try {
    ...
    const hookCmd = `node "${path.join(destHooksDir, 'codecortex-hook.cjs').replace(/\\/g, '/')}"`;
    ...
    const hasPreToolHook = existing.hooks.PreToolUse.some(
      (h: any) => h.hooks?.some((hh: any) => hh.command?.includes('codecortex'))
    );
    if (!hasPreToolHook) {
      existing.hooks.PreToolUse.push({
        ...
        hooks: [{
          ...
          command: hookCmd,
          timeout: 8000,
          statusMessage: 'Enriching with CodeCortex graph context...',
        }],
      });
    }
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    config.mcp.codecortex = getMcpEntry();
...
const SKILL_NAMES = ['codecortex-exploring', 'codecortex-debugging', 'codecortex-impact-analysis', 'codecortex-refactoring', 'codecortex-guide', 'codecortex-cli'];

/**
 * Install CodeCortex skills to a target directory.
 * Each skill is installed as {targetDir}/codecortex-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/codecortex/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/codecortex/
 */
...
export const setupCommand = async () => {
  ...
  console.log('    2. Run: codecortex analyze');
