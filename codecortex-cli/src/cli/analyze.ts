 ...
 * Analyze Command
 *
 * Indexes a repository and stores the knowledge graph in .codecortex/
 */
...
import { getStoragePaths, saveMeta, loadMeta, addToGitignore, registerRepo, getGlobalRegistryPath, saveHealthSnapshot } from '../storage/repo-manager.js';
import { getCurrentCommit, isGitRepo, getGitRoot } from '../storage/git.js';
import { computeHealthMetrics } from '../core/graph/health-calculator.js';
...
export const analyzeCommand = async (
  ...
  updateBar(99, 'Computing health metrics...');
  try {
    const healthSnapshot = await computeHealthMetrics(kuzuPath, { currentCommit });
    await saveHealthSnapshot(storagePath, healthSnapshot);
  } catch (err) {
  ...
  try {
    ...
    console.log('\n  Tip: Run `codecortex setup` to configure MCP for your editor.');
  }
