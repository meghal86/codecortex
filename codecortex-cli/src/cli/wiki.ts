 ...
 * Usage: codecortex wiki [path] [options]
 */
...
export const wikiCommand = async (
  ...
  if (!meta) {
    console.log('  Error: No CodeCortex index found.');
    console.log('  Run `codecortex analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }

  // ── Resolve LLM config (with interactive fallback) ─────────────────
  // Skip if we are running in structural-only mode
  let llmConfig: any = {};
  if (!options?.structuralOnly) {
    ...
    if (options?.apiKey || options?.model || options?.baseUrl) {
      ...
      console.log('  Config saved to ~/.codecortex/config.json\n');
    }
    ...
    if (!hasSavedConfig && !hasCLIOverrides) {
      if (!process.stdin.isTTY) {
        if (!llmConfig.apiKey) {
          ...
          console.log('  Set OPENAI_API_KEY or CODECORTEX_API_KEY environment variable,');
      ...
      const envKey = process.env.CODECORTEX_API_KEY || process.env.OPENAI_API_KEY || '';
      let key: string;
      if (envKey) {
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `codecortex wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync('gh', [
      ...
      '--desc', 'Repository Wiki — generated by CodeCortex',
      '--public',
