...
export class WikiGenerator {
  ...
  async run(): Promise<{ pagesGenerated: number; mode: 'full' | 'incremental' | 'up-to-date'; failedModules: string[] }> {
    ...
    try {
      if (this.options.structuralOnly) {
        const { StructuralRenderer } = await import('./structural-renderer.js');
        result = await new StructuralRenderer(this).run(currentCommit);
      } else if (!forceMode && existingMeta && existingMeta.fromCommit) {
