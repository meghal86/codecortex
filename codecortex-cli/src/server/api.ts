 ...
 * HTTP API Server
 *
 * REST API for browser-based clients to query the local .codecortex/ index.
...
import { loadMeta, listRegisteredRepos, loadHealthHistory } from '../storage/repo-manager.js';
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  app.use(cors({
    origin: (origin, callback) => {
      if (
        ...
        || origin === 'https://codecortex.vercel.app'
      ) {
        callback(null, true);
      } else {
  ...
  app.get('/api/repo', async (req, res) => {
    try {
      ...
      if (!entry) {
        res.status(404).json({ error: 'Repository not found. Run: codecortex analyze' });
        return;
      }
  ...
  app.get('/api/health-history', async (req, res) => {
    try {
      ...
      const history = await loadHealthHistory(entry.storagePath);
      res.json(history);
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Failed to get health history' });
    }
  });
  ...
  const server = app.listen(port, host, () => {
    console.log(`CodeCortex server running on http://${host}:${port}`);
  });
