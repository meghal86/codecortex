...
function closeOne(repoId: string): void {
  ...
  for (const conn of entry.available) {
    try { conn.close(); } catch (e) { console.error('CodeCortex [pool:close-conn]:', e instanceof Error ? e.message : e); }
  }
  try { entry.db.close(); } catch (e) { console.error('CodeCortex [pool:close-db]:', e instanceof Error ? e.message : e); }
  pool.delete(repoId);
}
 ...
 * Retries on lock errors (e.g., when `codecortex analyze` is running).
 */
export const initKuzu = async (repoId: string, dbPath: string): Promise<void> => {
  ...
  try {
    ...
    throw new Error(`KuzuDB not found at ${dbPath}. Run: codecortex analyze`);
  }
  ...
  // avoids lock conflicts when `codecortex analyze` is writing.
