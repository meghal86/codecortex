 ...
 * All resources use repo-scoped URIs: codecortex://repo/{name}/context
 */
...
export function getResourceDefinitions(): ResourceDefinition[] {
  return [
    ...
    {
      uri: 'codecortex://setup',
      name: 'CodeCortex Setup Content',
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'codecortex://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codecortex://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codecortex://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codecortex://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codecortex://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codecortex://repo/{name}/process/{processName}',
      name: 'Process Trace',
      description: 'Step-by-step execution trace',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codecortex://repo/{name}/health-history',
      name: 'Architecture Health History',
      description: 'Historical snapshots of architectural health metrics',
      mimeType: 'text/yaml',
    },
  ];
}
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'codecortex://repos') return { resourceType: 'repos' };
  if (uri === 'codecortex://setup') return { resourceType: 'setup' };

  // Repo-scoped: codecortex://repo/{name}/context
  const repoMatch = uri.match(/^codecortex:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: codecortex analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# codecortex_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: codecortex analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx codecortex analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - codecortex://repos: All indexed repositories');
  lines.push(`  - codecortex://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - codecortex://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - codecortex://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - codecortex://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: codecortex analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use codecortex_query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: codecortex analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use codecortex_query for deeper search.`);
    }
...
function getSchemaResource(): string {
  return `# CodeCortex Graph Schema
 ...
 * Useful for `codecortex setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# CodeCortex\n\nNo repositories indexed. Run: `npx codecortex analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# CodeCortex MCP — ${repo.name}`,
      '',
      `This project is indexed by CodeCortex as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`codecortex://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`codecortex://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`codecortex://repo/${repo.name}/processes\` — All execution flows`,
      `- \`codecortex://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
