 ...
 * 
 * Provides tool implementations using local .codecortex/ indexes.
...
// AI context generation is CLI-only (codecortex analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`CodeCortex [${context}]: ${msg}`);
}
...
export class LocalBackend {
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: codecortex analyze');
    }
    if (repoParam) {
  ...
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    ...
    try {
      ...
      console.error('CodeCortex: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
      return [];
    }
