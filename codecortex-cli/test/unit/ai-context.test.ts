...
describe('generateAIContextFiles', () => {
  ...
  beforeAll(async () => {
    ...
    storagePath = path.join(tmpDir, '.codecortex');
  ...
  it('creates or updates CLAUDE.md with CodeCortex section', async () => {
  ...
  it('updates existing CLAUDE.md without duplicating', async () => {
    ...
    // Should only have one codecortex section
    const starts = (content.match(/codecortex:start/g) || []).length;
  ...
  it('installs skills files', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills', 'codecortex');
    try {
