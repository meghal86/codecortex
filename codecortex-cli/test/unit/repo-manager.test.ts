...
describe('getStoragePath', () => {
  it('appends .codecortex to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.codecortex');
    expect(path.basename(result)).toBe('.codecortex');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, kuzuPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.codecortex');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.codecortex/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('codecortex-config-test-');
